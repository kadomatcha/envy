...
class APIDocumentation {
    ...
    updatePageContent() {
        ...
        if (copyright) {
            copyright.textContent = `© ${year} ${this.settings.creator || 'EnvyXml'}. All rights reserved.`;
        }
        ...
        // Updata response time
        const responseTimeElement = document.getElementById('responseTime');
if (responseTimeElement) {
  const updateFakeResponseTime = () => {
    const randomMs = Math.floor(Math.random() * 300) + 10; // angka acak 50–149 ms
    responseTimeElement.textContent = `${randomMs}ms`;
  };

  updateFakeResponseTime();
  setInterval(updateFakeResponseTime, 500);
}
    ...
    for (const [key] of urlParams) {
        ...
        if (api.params && api.params[key]) {
            const paramDef = api.params[key];
            // Handle both string (legacy) and object formats
            const isObjectFormat = typeof paramDef === 'object';
            params.push({
                name: key,
                description: isObjectFormat ? paramDef.description : paramDef,
                required: isObjectFormat ? (paramDef.required !== false) : true // default to true
            });
        } else {
            // Fallback for parameters not defined in the params object
            params.push({
                name: key,
                description: '',
                required: true // default to required if not defined
            });
        }
    }
    ...
    const paramsHTML = `
        ...
        ${params.map(param => `
            <div class="param-item">
                ...
                <input 
                    type="text" 
                    class="param-input ${param.required ? 'required' : 'optional'}" 
                    id="param-${param.name}"
                    placeholder="${param.required ? `Enter ${param.name} (required)` : `Enter ${param.name} (optional)`}"
    ...
    this.elements.paramsSection.querySelectorAll('.param-input[required]').forEach(input => {
